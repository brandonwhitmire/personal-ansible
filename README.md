...
```
FILES=(~/<FILE1> ~/<FILE2>); DESTINATION_USER="<USER>"; DESTINATION_HOST="<IP_ADDRESS>"; CONTROL_PATH="/tmp/ssh-multiplex-%r@%h:%p"; FAIL_FLAG=0; ssh -o ControlMaster=yes -o ControlPath="${CONTROL_PATH}" -o ControlPersist=5m "${DESTINATION_USER}@${DESTINATION_HOST}" "exit" || { echo "Failed to establish SSH connection"; FAIL_FLAG=1; }; if [ $FAIL_FLAG -eq 0 ]; then for FILE in "${FILES[@]}"; do FILE_DIR=$(dirname "${FILE}"); ssh -o ControlPath="${CONTROL_PATH}" "${DESTINATION_USER}@${DESTINATION_HOST}" "mkdir -p \"${FILE_DIR}\"" || { echo "Failed to create directory ${FILE_DIR} on ${DESTINATION_HOST}"; FAIL_FLAG=1; break; }; rsync -avz -e "ssh -o ControlPath=${CONTROL_PATH}" "${FILE}" "${DESTINATION_USER}@${DESTINATION_HOST}:${FILE_DIR}/" || { echo "Failed to transfer ${FILE}"; FAIL_FLAG=1; break; }; done; fi; if [ $FAIL_FLAG -eq 0 ]; then ssh -O exit -o ControlPath="${CONTROL_PATH}" "${DESTINATION_USER}@${DESTINATION_HOST}" || { echo "Failed to close SSH connection"; }; else echo "Script aborted due to earlier errors."; fi
```
